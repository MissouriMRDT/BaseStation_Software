...
export async function parse(packet: Buffer, rinfo?: any) {
  ...
  if (version === VersionNumber) {
    ...
    rovecomm.emit(
      'all',
      `Packet with Rovecomm version of ${version} recieved. Incorrect version. Aborted.`
    )
  }
}
...
class Rovecomm extends EventEmitter {
  ...
  sendUDP(packet: Buffer, destinationIp: string, port = ethernetUDPPort): void {
     ...
     */
    // eslint-disable-next-line @typescript-eslint/no-use-before-define
    console.log(`Destination: ${destinationIp}:${port}`)
  ...
  sendCommand(
    ...
    if (Object.prototype.hasOwnProperty.call(RovecommManifest, boardName)) {
      if (dataIdStr in RovecommManifest[boardName].Commands) {
        destinationIp = testLocally ? '127.0.0.1' : RovecommManifest[boardName].Ip
    ...
    if (reliability === false) {
      console.log('Sending UDP Command')
      this.sendUDP(packet, destinationIp, testLocally ? 11001 : ethernetUDPPort)
    } else {
